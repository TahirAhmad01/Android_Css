$(function () {
  ...
  function music_play_nm(index_no) {
    ...
    $("#playback_image").attr(
      "src",
      "../img/music/" + audio_track_list[index_no].image
    );
  ...
  for (let i = 0; i < audio_track_list.length; i++) {
    $(".playlist").append(
      "<div class='playlist_item p_song_title_" +
        i +
        "'> <div>" +
        audio_track_list[i].name +
        "</div> <div id='p_song_artist' class='p_artist'> Artist: " +
        audio_track_list[i].artist +
        "</div> </div>"
    );
  ...
  setInterval(() => {
    ...
    if (track.currentTime == track.duration) {
      if (
        $("#auto_play").hasClass("auto_active") &&
        $(".play").hasClass("active") &&
        index_no < audio_track_list.length
      ) {
        index_no += 1;
        music_play_nm(index_no);
        play();
